...
public class GenerateCube : MonoBehaviour
{
    ...
    private void SetVertices()
    {
        ...
        var faceCount = (_xSize - 1) * (_ySize - 1) + (_xSize - 1) * (_zSize - 1) + (_ySize - 1) * (_zSize - 1);
        faceCount *= 2;
        _vertices = new Vector3[cornerCount + edgeCount + faceCount];

        var v = 0;
        for (float y = -1 * (float)_ySize / 2; y <= (float)_ySize / 2; y++)
        {
            for (float x = -1 * (float)_xSize / 2; x <= (float)_xSize / 2; x++)
            {
                _vertices[v++] = new Vector3(x, y, -1 * (float)_zSize / 2);
            }

            for (float z = -1 * (float)_zSize / 2 + 1; z <= (float)_zSize / 2; z++)
            {
                _vertices[v++] = new Vector3((float)_xSize / 2, y, z);
            }

            for (float x = (float)_xSize / 2 - 1; x >= -1 * (float)_xSize / 2; x--)
            {
                _vertices[v++] = new Vector3(x, y, (float)_zSize / 2);
            }

            for (float z = (float)_zSize / 2 - 1; z > -1 * (float)_zSize / 2; z--)
            {
                _vertices[v++] = new Vector3(-1 * (float)_xSize / 2, y, z);
            }
        }

        for (float z = -1 * (float)_zSize / 2 + 1; z < (float)_zSize / 2; z++)
        {
            for (float x = -1 * (float)_xSize / 2 + 1; x < (float)_xSize / 2; x++)
            {
                _vertices[v++] = new Vector3(x, (float)_ySize / 2, z);
            }
        }
        ...
        for (float z = -1 * (float)_zSize / 2 + 1; z < (float)_zSize / 2; z++)
        {
            for (float x = -1 * (float)_xSize / 2 + 1; x < (float)_xSize / 2; x++)
            {
                _vertices[v++] = new Vector3(x, -1 * (float)_ySize / 2, z);
            }
        }
    ...
    private int SetTop(int[] triangles, int ti, int loop)
    {
        ...
        {
            ti = SetCell(triangles, ti, v_out_right, v_out_right + loop - 1, v_out_right + 1, v_out_right + loop);
        }
        ti = SetCell(triangles, ti, v_out_right, v_out_right + loop - 1, v_out_right + 1, v_out_right + 2);
        //Inner rows
        v_out_right += 2;
    ...
    private void SetNormals()
    {
        ...
        var centerPoint = new Vector3((float)_xSize / 16, (float)_ySize / 16, (float)_zSize / 16);
    ...
    //    Gizmos.DrawSphere(_vertices[0], 0.1f);
